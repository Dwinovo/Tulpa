...
public class NumenMod {
    ...
    public NumenMod() {
        ...
        MinecraftForge.EVENT_BUS.addListener(NumenMod::onServerTickPost);
    ...
    private static void onServerTickPost(TickEvent.ServerTickEvent event) {
        ...
        com.dwinovo.numen.task.tasks.ScanBlocksJob.tick(server);
        com.dwinovo.numen.task.CompanionTickDispatcher.tick(server);
        // Snapshot loaded chunks near companions each tick, for the off-thread planner to read live.
        com.dwinovo.numen.pathing.cache.PathCaches.serverTick(server);
    }
